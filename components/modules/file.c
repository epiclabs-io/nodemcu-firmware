...
static int file_close(lua_State *L) {
    ...
    if (ud->fd) {
        if (vfs_close(ud->fd) != VFS_RES_OK) {
            luaL_error(L, "Error closing file");
            // does not return
        }
        // mark as closed
        ud->fd = 0;
...
static int32_t standard_vfs_close(const struct vfs_file *fd) {
    return VFS_RES_ERR;
}

static int32_t standard_vfs_read(const struct vfs_file *fd, void *ptr, size_t len) {
    return VFS_RES_ERR;
}

static int32_t standard_vfs_write(const struct vfs_file *fd, const void *ptr, size_t len) {
    size_t written = fwrite(ptr, 1, len, stdout);
    return (written == len) ? written : VFS_RES_ERR;
}

static int32_t standard_vfs_lseek(const struct vfs_file *fd, int32_t off, int whence) {
    return VFS_RES_ERR;
}

static int32_t standard_vfs_eof(const struct vfs_file *fd) {
    return VFS_RES_ERR;
}

static int32_t standard_vfs_tell(const struct vfs_file *fd) {
    return VFS_RES_ERR;
}

static int32_t standard_vfs_flush(const struct vfs_file *fd) {
    return VFS_RES_ERR;
}

static uint32_t standard_vfs_size(const struct vfs_file *fd) {
    return VFS_RES_ERR;
}

static int32_t standard_vfs_ferrno(const struct vfs_file *fd) {
    return VFS_RES_ERR;
}

static vfs_file_fns standard_file_fns = {
    .close = standard_vfs_close,
    .read = standard_vfs_read,
    .write = standard_vfs_write,
    .lseek = standard_vfs_lseek,
    .eof = standard_vfs_eof,
    .tell = standard_vfs_tell,
    .flush = standard_vfs_flush,
    .size = standard_vfs_size,
    .ferrno = standard_vfs_ferrno};
...
static struct vfs_file file_stdout = {
    .fs_type = 44,
    .fns = &standard_file_fns};

int luaopen_file(lua_State *L) {
    luaL_rometatable(L, "file.obj", (void *)file_obj_map);

    file_fd_ud *ud = (file_fd_ud *)lua_newuserdata(L, sizeof(file_fd_ud));
    ud->fd = (int)&file_stdout;
    luaL_getmetatable(L, "file.obj");
    lua_setmetatable(L, -2);
